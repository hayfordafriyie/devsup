...
public sealed class DashboardTests : IAsyncLifetime
{
    ...
    public async Task Dashboard_DeliveryCenterHooksPresent()
    {
        ...
        Assert.Contains("data-member-transfer", js);
        Assert.Contains("data-repo-leave", js);
        ...
        Assert.Contains("repo-activity-panel", html2);
        Assert.Contains("data-repo-activity", js);
        ...
        Assert.Contains("quietHoursStart", js);
        Assert.Contains("quietHoursEnd", js);
        Assert.Contains("repo-bulk-bar", html2);
        Assert.Contains("data-bulk-action", js);
        Assert.Contains("repo-select", js);
        Assert.Contains("updateBulkBar", js);
        Assert.Contains("\"/api/repositories/bulk\"", js);
    }
}